...
//        });
	...
	function saveToDb2($json) {
		...
		foreach ($list as $li) {
			$data = array();
			foreach ($fields_client as $v) {
				if (isset($li['client_info'][$v])) {
					$data['client_' . $v] = $li['client_info'][$v];
				}
			}
			foreach ($fields_http as $v) {
				if (isset($li['http'][$v])) {
					$data['http_' . $v] = $li['http'][$v];
				}
			}
			if (isset($li['http']['url'])) {
				if ($r = $this->parserUrl($li['http']['url'])) {
					if (isset($r['host'])) {
						$data['http_host'] = $r['host'];
					}
					if (isset($r['path'])) {
						$data['http_uri'] = $r['path'];
					}
					if (isset($r['get']['app'])) {
						$data['http_app'] = $r['get']['app'];
					}
					if (isset($r['get']['appVersion'])) {
						$data['http_ver'] = $r['get']['appVersion'];
					}
				}
			}
			table('st_memtemp')->put($data);
		}
	}
